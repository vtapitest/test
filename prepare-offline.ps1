...
# Crear scripts sin here-strings - usando arrays de líneas
Write-Host "Creando scripts de instalación..." -ForegroundColor Yellow
...
$installLines -join "`n" | Set-Content -Path "$PACKAGE_DIR/install.sh" -Encoding UTF8
...
$initLines -join "`n" | Set-Content -Path "$PACKAGE_DIR/init-app.sh" -Encoding UTF8
...
$restartLines -join "`n" | Set-Content -Path "$PACKAGE_DIR/restart.sh" -Encoding UTF8
...
$backupLines -join "`n" | Set-Content -Path "$PACKAGE_DIR/backup-db.sh" -Encoding UTF8
...
$restoreLines -join "`n" | Set-Content -Path "$PACKAGE_DIR/restore-db.sh" -Encoding UTF8
...
$readmeLines -join "`n" | Set-Content -Path "$PACKAGE_DIR/README.md" -Encoding UTF8
